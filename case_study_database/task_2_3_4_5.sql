...
-- where round(year(datediff(curdate - ngay_sinh)) >=18 and round(year(datediff(curdate - ngay_sinh)) <=50;
...
sum(coalesce(dich_vu.chi_phi_thue,0) + coalesce((hop_dong_chi_tiet.so_luong * dich_vu_di_kem.gia),0)) as tong_tien
from khach_hang
